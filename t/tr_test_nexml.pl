...
my $infile = "data/tr_nexml_test02.xml";
my $blocks = parse(
    '-file'   => $infile,
    '-format' => 'nexml',
    );
...
for my $block ( @{ $blocks } ) {
	...
	foreach my $tree ( @{ $forest->get_entities } ) {
	    ...
	    my $tree_metadata = $tree->get_meta();
	    foreach my $tree_meta ( @{ $tree_metadata } ) {
		print STDERR $tree_meta->get_predicate."-->";
		print STDERR $tree_meta->get_object."\n";
	    }
		...
		my $node_xml_id = $node->get_xml_id;
		print STDERR "\tXMLID: ".$node_xml_id."\n";
